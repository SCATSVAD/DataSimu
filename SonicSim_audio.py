...
def all_pairs(
    ...
    --> list1_pair = [1, 1, 2, 2, 3, 3]  list2_pair = ['a', 'b', 'a', 'b', 'a', 'b']
...
def create_ali_audio(spk2path, length, rttm, sample_rate=16000):
    ...
    audio_path_list = []
    for root, _, files in os.walk(str(audio_path)):
        for file in files:
            if file.endswith('.flac'):
                audio_path_list.append(os.path.join(root, file))
    print(f"audio_path_list: {len(audio_path_list)}")
    spk_rttm = [line for line in rttm if line['spk_id'] == spk] 
    long_audio = torch.zeros((1, int(length * sample_rate)))
    
    for line in spk_rttm:
        st = line['st']
        dt = line['dt']
        reco = line['reco']
        start_index = int(st * sample_rate)
        sig_dt = int(dt * sample_rate)
        if start_index + sig_dt >= int(length * sample_rate):
            with open(f"{reco}.txt", "w", encoding="utf-8") as file:
                file.write(reco)
            continue
